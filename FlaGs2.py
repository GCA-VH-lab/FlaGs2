...
if not args.tree_order:
	...
	for m in eg1:
		if m != '':
			...
			ndoms=len(entries1)
			y_level_m = y_level_m-5-round(postscriptSize(newQ))
			for entry in entries1:
				...
				arrowList.append(ax[1].arrow(x=x_gene_start, y=y_level_m, dx=dx_gene_length, dy=0, width=3.5, head_width=3.5, length_includes_head = True, head_length = 150, facecolor = colorDict[dom1_name], edgecolor = outliner(colorDict[dom1_name]), alpha=1))
				...
				if dom1_name != 0 and x_gene_start != 1 and 'pseudogene_' not in id1 and 'tRNA_' not in id1:
					ax[1].text(text_x, y_level_m-0.8, s = dom1_name, horizontalalignment='center', color = 'k', font = {'family' : 'sans-serif','size'   : 8})
				else:
					pass
				...
				ax[0].text(0.7, y_level_m, org, horizontalalignment='center', color = '#000000', font = {'family' : 'sans-serif','size':10})
				ax[0].set_axis_off()
	...
	plt.axis('off')
	plt.tight_layout()
	plt.savefig(args.out_prefix+'_operon.pdf', format='pdf', bbox_inches='tight') #For FlaGs input
...
if args.tree and args.tree_order:  # Queries in postscript file will be presented as tree order
	...
	plt.axis('off')
	plt.tight_layout()
	plt.savefig(args.out_prefix+'_TreeOrder_output.pdf', format='pdf', bbox_inches='tight') #For FlaGs input
