...
class ReportWriter: 
  ...
  def outdesc_txt(self, path):
    # Block order matches main: families by total occurrences descending, members
    # left in cluster order. Labels stay the global ones used by the figures.
...
def write_run_info(args, parser):
  """Record how the run was invoked, next to its results.

  Written before any work starts, so a run that dies partway still leaves a
  ...
  try:
    shutil.copyfile(args.input_list, out_path("_input.txt"))
  except OSError as e:
    print("Warning: could not copy the input list into the output directory "
        "({}).".format(e))
...
def main():
  ...
  parser.add_argument("--no_cross_db", action="store_true", help=" Keep protein and genome in the same database: RefSeq proteins (WP_, NP_, ...) resolve only to GCF_ assemblies and INSDC proteins only to GCA_. Proteins whose only assemblies are in the other database are then reported as unresolved. Does not affect assemblies given explicitly in the input file. ")
  ...
  parser.add_argument("--iqtree", action="store_true", help=" Build the tree with IQ-TREE (ModelFinder plus 1000 ultrafast bootstrap replicates) instead of VeryFastTree. Implies --tree. Much slower but gives model selection and branch support. Needs iqtree on PATH. ")
  ...
  if tree_written:
    print("  {}_tree.svg / {}_tree.nwk / {}_tree.aln".format(prefix, prefix, prefix))
  elif want_tree:
