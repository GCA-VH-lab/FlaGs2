...
if args.hmmdb:
	...
	if args.cpu:
		hmmscan_cmd = "hmmscan -E 0.00000001 --cpu %s -o %s_dom.txt --domtblout %s_dom_out.txt %s %s_flankgene.fasta_cluster_out"%(round(core/3), args.out_prefix, args.out_prefix, args.hmmdb, args.out_prefix)
	else:
		hmmscan_cmd = "hmmscan -E 0.00000001 -o %s_dom.txt --domtblout %s_dom_out.txt %s %s_flankgene.fasta_cluster_out"%(args.out_prefix, args.out_prefix, args.hmmdb, args.out_prefix)
		...
		for line in dom_dict_infile:
			if any(e in line for e in seqDict.keys()):
				...
				if dom_dict_line[3]!=dom_key:
					...
					dom_list=[]
					dom_value = "\t"+"\t"+dom_dict_line[0]+"\t"+dom_dict_line[17]+"\t"+dom_dict_line[18]+"\n"
					dom_list.append(dom_value)
					dom_dict[dom_key_cut[0]]=dom_list
				else:
					dom_value = "\t"+"\t"+dom_dict_line[0]+"\t"+dom_dict_line[17]+"\t"+dom_dict_line[18]+"\n"
					dom_list.append(dom_value)
					dom_dict[dom_key_cut[0]]=dom_list
